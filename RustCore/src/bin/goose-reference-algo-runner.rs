...
) -> goosecore::GooseResult<ReferenceRun> {
...
fn run_typed<I, O>(
    ...
    record: impl FnOnce(&str, &AlgorithmRunResult<O>) -> goosecore::GooseResult<AlgorithmRunRecord>,
) -> goosecore::GooseResult<ReferenceRun>
where
    I: DeserializeOwned,
    O: Serialize,
...
fn external_reference_definition(
    report: &ExternalReferenceOutput,
) -> goosecore::GooseResult<AlgorithmDefinitionRecord> {
...
fn values(args: &[String], name: &str) -> goosecore::GooseResult<Vec<String>> {
...
fn require_non_empty(field: &str, value: &str) -> goosecore::GooseResult<()> {
...
fn require_object(field: &str, value: &serde_json::Value) -> goosecore::GooseResult<()> {
    if !value.is_object() {
        return Err(GooseError::message(format!(
...
fn require_array(field: &str, value: &serde_json::Value) -> goosecore::GooseResult<()> {
    if !value.is_array() {
        return Err(GooseError::message(format!(
...
fn reference_input_path(
    args: &[String],
    family: &str,
) -> goosecore::GooseResult<std::path::PathBuf> {
    match family {
        "hrv" => default_path(
            args,
            "--input",
