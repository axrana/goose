...
fn raw_export_bundle_path_value(args: &[String]) -> goosecore::GooseResult<Option<PathBuf>> {
...
) -> goosecore::GooseResult<ResolvedValidationDatabase> {
...
fn sqlite_table_exists(connection: &Connection, table_name: &str) -> goosecore::GooseResult<bool> {
    connection
        .query_row(
...
fn audit_raw_export_zip_manifest(
    ...
    let manifest_raw = (|| -> goosecore::GooseResult<String> {
...
fn file_sha256_hex(path: &Path) -> goosecore::GooseResult<String> {
...
fn extract_raw_export_sqlite_from_zip(
    bundle_path: &Path,
) -> goosecore::GooseResult<ExtractedRawExportSqlite> {
    let file = File::open(bundle_path).map_err(|source| GooseError::io(bundle_path, source))?;
    let mut archive = ZipArchive::new(file).map_err(|error| {
        GooseError::message(format!(
            "raw export bundle {} is not a readable zip: {error}",
...
fn read_manifest_value(path: &Path) -> goosecore::GooseResult<Value> {
...
fn parse_manifest(value: Value) -> goosecore::GooseResult<LocalHealthValidationManifest> {
...
fn write_markdown_report(
    ...
    path: &Path,
) -> goosecore::GooseResult<()> {
    write_markdown_text(&markdown_report(report), path)
}

fn write_json_file<T: Serialize>(report: &T, path: &Path) -> goosecore::GooseResult<()> {
...
fn write_markdown_text(markdown: &str, path: &Path) -> goosecore::GooseResult<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|source| GooseError::io(parent, source))?;
    }
...
) -> goosecore::GooseResult<Option<PathBuf>> {
...
fn copy_capture_session_scope(
    ...
    let copy_result = (|| -> goosecore::GooseResult<()> {
        connection
            .execute_batch(&format!(
                r#"
...
fn source_table_exists(connection: &Connection, table_name: &str) -> goosecore::GooseResult<bool> {
    connection
        .query_row(
...
fn persist_scoped_formatted_metric_writes(
    ...
    let copy_result = (|| -> goosecore::GooseResult<usize> {
