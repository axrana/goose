...
) -> goosecore::GooseResult<AlgorithmComparisonReport> {
    match family {
        "hrv" => {
...
fn run_benchmark_algorithm(
    algorithm_id: &str,
    input_path: &PathBuf,
) -> goosecore::GooseResult<BenchmarkRun> {
    match algorithm_id {
...
fn benchmark_label_args(args: &[String]) -> goosecore::GooseResult<Option<BenchmarkLabelArgs>> {
...
fn read_typed_input<I>(input_path: &PathBuf) -> goosecore::GooseResult<I>
where
    I: DeserializeOwned,
{
    let input_raw =
...
fn read_json_value(path: &PathBuf) -> goosecore::GooseResult<serde_json::Value> {
...
fn input_path(args: &[String], algorithm_id: &str) -> goosecore::GooseResult<PathBuf> {
...
fn input_path_for_family(args: &[String], family: &str) -> goosecore::GooseResult<PathBuf> {
    if let Some(path) = path_value(args, "--input")? {
        return Ok(path);
    }
    match family {
