...
fn release_gate_input_from_report_paths(
    args: &[String],
) -> goosecore::GooseResult<SleepV1ReleaseGateInput> {
...
fn optional_json<T: serde::de::DeserializeOwned>(
    args: &[String],
    name: &str,
) -> goosecore::GooseResult<Option<T>> {
    path_value(args, name)?.map_or(Ok(None), |path| read_json(&path).map(Some))
}

fn benchmark_reports(args: &[String]) -> goosecore::GooseResult<Vec<AlgorithmComparisonReport>> {
...
fn optional_usize(args: &[String], name: &str) -> goosecore::GooseResult<Option<usize>> {
...
fn read_json<T: serde::de::DeserializeOwned>(path: &Path) -> goosecore::GooseResult<T> {
