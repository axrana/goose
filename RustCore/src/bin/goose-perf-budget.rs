    ...
    Ok(value(args, name)?
        .map(|raw| {
            raw.parse::<usize>().map_err(|source| {
                goosecore::GooseError::message(format!("invalid {name}: {source}"))
            })
        })
        .transpose()?)
}

fn optional_u64(args: &[String], name: &str) -> goosecore::GooseResult<Option<u64>> {
    Ok(value(args, name)?
        .map(|raw| {
...
fn optional_mib(args: &[String], name: &str) -> goosecore::GooseResult<Option<u64>> {
