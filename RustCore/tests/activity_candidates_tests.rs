...
use goosecore::activity_candidates::{
...
use goosecore::activity_sessions::{
    ACTIVITY_SESSION_CORRECTION_SCOPE, activity_session_correction_plans,
};
use goosecore::health_sync::{ActivitySyncCandidate, ActivitySyncMetric, HealthSyncSessionKind};
use serde_json::json;
...
fn assert_packet_derived_plan_report(
    ...
    assert_eq!(
        report.schema,
        goosecore::activity_candidates::ACTIVITY_SESSION_PACKET_DERIVED_METRIC_PLAN_REPORT_SCHEMA
    );
    assert_eq!(
        report.generated_by,
        goosecore::activity_candidates::ACTIVITY_SESSION_PACKET_DERIVED_METRIC_PLAN_GENERATED_BY
    );
